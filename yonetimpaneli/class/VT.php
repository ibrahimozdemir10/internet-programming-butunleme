    ...
    public function kategoriGetir($tablo,$secID="",$uz=-1)
    {
        ...
        if ($kategori!=false)
        {
            for($q=0;$q<count($kategori);$q++)
            {
                ...
                if($secID==$kategoriID)
                {
                    echo '<option value="'.$kategoriID.'" selected="selected">'.str_repeat("&nbsp;&nbsp;&nbsp;",$uz).stripslashes($kategoriID[$q]["baslik"]).'</option>';
                }
                else
                {
                    echo '<option value="'.$kategoriID.'" >'.str_repeat("&nbsp;&nbsp;&nbsp;",$uz).stripslashes($kategoriID[$q]["baslik"]).'</option>';              
                }
